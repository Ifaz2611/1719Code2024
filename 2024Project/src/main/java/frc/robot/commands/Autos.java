...
public static Command Red_SK(DeviceSubsystem m_DeviceSubsystem, ShooterAnglePIDSubsystem m_AnglePIDSubsystem, LimelightSubsystem m_limelight, SwerveSubsystem m_swerveDrive) {
    return new SequentialCommandGroup(
        new WaitCommand(.5),
        //ONE NOTE
        new PIDGyroCommand(60, m_swerveDrive).withTimeout(.5), //turn 60, to speaker
        new PIDCommandTurnToAngle(m_limelight, m_swerveDrive).withTimeout(1), //turn to limelight to ensure alignment
        new ShootSequence(m_DeviceSubsystem).withTimeout(2.4), //shoot

        //TWO NOTE
        new PIDGyroCommand(0, m_swerveDrive).withTimeout(1), //turn to 0
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "intakeOn", Constants.
        MAX_SHOOTER_ANGLE), //turn on intake
        new WaitCommand(1), //wait for arm to reach to ground (approx 1 second)
        new AutoMovePIDCommand(0, 50, m_swerveDrive).until(m_DeviceSubsystem::checkRing).withTimeout(2), //move back 50 inches
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "intakeOff", Constants. //turn off intake
        MAX_SHOOTER_ANGLE).withTimeout(1),
        new PIDGyroCommand(30, m_swerveDrive).withTimeout(.8), //turn to speaker
        new PIDCommandTurnToAngle(m_limelight, m_swerveDrive).withTimeout(1), //turn to limelight
        new WaitCommand(1), //let arm auto aim (approx 1 second)
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "outtake", Constants.DEFAULT_SHOOTER_ANGLE), //outtake momentarily
        new ShootSequence(m_DeviceSubsystem).withTimeout(2.4), //shoot
    ...
    // new PIDGyroCommand(0, m_swerveDrive),
        new WaitCommand(100) //DO NOT COMMENT to avoid repeats
    );
  }

public static Command RedorBlue_SB_or_SJ(DeviceSubsystem m_DeviceSubsystem, ShooterAnglePIDSubsystem m_AnglePIDSubsystem, LimelightSubsystem m_limelight, SwerveSubsystem m_swerveDrive) {
    return new SequentialCommandGroup(
        new WaitCommand(.5), //let arm reach floor (start position at angle 0)
        //ONE NOTE
        new ShootSequence(m_DeviceSubsystem).withTimeout(2.4), //shoot
        //TWO NOTE
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "intakeOn", Constants. //turn on intake
        MAX_SHOOTER_ANGLE),
        new WaitCommand(1), //let arm reach floor (approx 1 second)
        new AutoMovePIDCommand(0, 40, m_swerveDrive).until(m_DeviceSubsystem::checkRing).withTimeout(2), //move back 50
        new PIDGyroCommand(0, m_swerveDrive).withTimeout(.5), //set robot straight
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "intakeOff", Constants. //turn off intake
        MAX_SHOOTER_ANGLE).withTimeout(1),
        new WaitCommand(1), //let arm auto aim (approx 1 second)
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "outtake", Constants.DEFAULT_SHOOTER_ANGLE), //outtake momentarily
...
public static Command Red_SI(DeviceSubsystem m_DeviceSubsystem, ShooterAnglePIDSubsystem m_AnglePIDSubsystem, LimelightSubsystem m_limelight, SwerveSubsystem m_swerveDrive) {
    return new SequentialCommandGroup(
        //AUTO LEFT
        new WaitCommand(.5), //let arm reach ground
        //ONE NOTE
        new PIDGyroCommand(-60, m_swerveDrive).withTimeout(.5), //turn to speaker
        ...
        new PIDGyroCommand(0, m_swerveDrive).withTimeout(1), //turn to 0
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "intakeOn", //turn on intake
        Constants.MAX_SHOOTER_ANGLE),
        new WaitCommand(1), //let arm reach ground
        new AutoMovePIDCommand(0, 40, m_swerveDrive).until(m_DeviceSubsystem::checkRing).withTimeout(2), //move back to ring
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "intakeOff", Constants.
        MAX_SHOOTER_ANGLE).withTimeout(1), //stop intake

        new PIDGyroCommand(-30, m_swerveDrive).withTimeout(.8), //turn to speaker
        new PIDCommandTurnToAngle(m_limelight, m_swerveDrive).withTimeout(.5), //align to april tag
        new WaitCommand(1), //auto aim
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "outtake", Constants.DEFAULT_SHOOTER_ANGLE), //outtake momentarily
        new ShootSequence(m_DeviceSubsystem).withTimeout(2.4), //shoot
...
public static Command Blue_SA(DeviceSubsystem m_DeviceSubsystem, ShooterAnglePIDSubsystem m_AnglePIDSubsystem, LimelightSubsystem m_limelight, SwerveSubsystem m_swerveDrive) {
    return new SequentialCommandGroup(
        new WaitCommand(.5),
        //ONE NOTE
...
public static Command Blue_SCH(DeviceSubsystem m_DeviceSubsystem, ShooterAnglePIDSubsystem m_AnglePIDSubsystem, LimelightSubsystem m_limelight, SwerveSubsystem m_swerveDrive) {
    return new SequentialCommandGroup(
        new WaitCommand(.5), //wait for arm to reach ground
        // Zero Gyro
        new InstantCommand(() -> {
            Robot.zeroGYRO();
        }),
        /* FIRST NOTE */
        new PIDGyroCommand(-60, m_swerveDrive).withTimeout(.5), // Turn to speaker degrees
        new PIDCommandTurnToAngle(m_limelight, m_swerveDrive).withTimeout(1), // Align to april tag
        new ShootSequence(m_DeviceSubsystem).withTimeout(1.7), // Shoot
        /* SECOND NOTE */
        new PIDGyroCommand(0, m_swerveDrive).withTimeout(1.25), // Turn to 0 degree
        ...
        new PIDCompositionDriveCommand(m_swerveDrive, -2, 45, -22).withTimeout(1.5), // Drive to 45 inches
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "intakeOff", Constants.MAX_SHOOTER_ANGLE).withTimeout(0.25), // Turn intake off
        //new PIDGyroCommand(-30, m_swerveDrive).withTimeout(1), // Turn to -30 degrees
        new PIDCommandTurnToAngle(m_limelight, m_swerveDrive).withTimeout(0.5), // Align to april tag
        new WaitCommand(1), // Wait to allow time for auto aim
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "outtake", Constants.DEFAULT_SHOOTER_ANGLE), // Outtake a little
        new ShootSequence(m_DeviceSubsystem).withTimeout(1.7), // Shoot
        /* THIRD NOTE */
        new PIDGyroCommand(0, m_swerveDrive).withTimeout(1.25), // Turn to 0 degrees
...
public static Command Red_SKH(DeviceSubsystem m_DeviceSubsystem, ShooterAnglePIDSubsystem m_AnglePIDSubsystem, LimelightSubsystem m_limelight, SwerveSubsystem m_swerveDrive) {
    return new SequentialCommandGroup(
        ...
        new PIDCompositionDriveCommand(m_swerveDrive, 0, 45, 0).withTimeout(1.5), // Drive to 45 inches
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "intakeOff", Constants.MAX_SHOOTER_ANGLE).withTimeout(0.25), // Turn intake off
        //new PIDGyroCommand(22, m_swerveDrive).withTimeout(1), // Turn to -22 degrees
        new PIDCommandTurnToAngle(m_limelight, m_swerveDrive).withTimeout(0.5), // Align to april tag
        new WaitCommand(1), // Wait to allow time for arm to adjust
        ...
        new ShootSequence(m_DeviceSubsystem).withTimeout(1.7), // Shoot
        /* THIRD NOTE */
        new PIDGyroCommand(0, m_swerveDrive).withTimeout(.5), // Turn to 0 degrees
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "intakeOn", Constants.MAX_SHOOTER_ANGLE), // Turn on intake
        new InstantCommand(() -> {
            m_swerveDrive.resetDistanceMotors(); // Reset distance motors (CURCIAL TO DRIVING)
        }),
...
public static Command Blue_SAB(DeviceSubsystem m_DeviceSubsystem, ShooterAnglePIDSubsystem m_AnglePIDSubsystem, LimelightSubsystem m_limelight, SwerveSubsystem m_swerveDrive) {
    return new SequentialCommandGroup(
        ...
        new PIDCompositionDriveCommand(m_swerveDrive, -145, 80, 0).withTimeout(1.5), // Drive to 45 inches
        new InstantCommand(() -> {
            m_swerveDrive.resetDistanceMotors(); // Reset distance motors (CURCIAL TO DRIVING)
        }),
        new PIDCompositionDriveCommand(m_swerveDrive, 0,50, 0).until(m_DeviceSubsystem::checkRing).withTimeout(1.5), // Drive to 45 inches
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "intakeOff", Constants.MAX_SHOOTER_ANGLE).withTimeout(0.25), // Turn intake off
        new WaitCommand(1), // Wait to allow time for arm to adjust
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "outtake", Constants.DEFAULT_SHOOTER_ANGLE), // Outtake a little
        new ShootSequence(m_DeviceSubsystem).withTimeout(1.7) // Shoot
...
public static Command Blue_SBC(DeviceSubsystem m_DeviceSubsystem, ShooterAnglePIDSubsystem m_AnglePIDSubsystem, LimelightSubsystem m_limelight, SwerveSubsystem m_swerveDrive) {
    return new SequentialCommandGroup(
        ...
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "setAngle", Constants.MAX_SHOOTER_ANGLE),
        /* FIRST NOTE */
        new ShootSequence(m_DeviceSubsystem).withTimeout(1.7), // Shoot
        /* SECOND NOTE */
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "intakeOn", Constants.MAX_SHOOTER_ANGLE), // Turn on intake
        ...
        new PIDCompositionDriveCommand(m_swerveDrive, -145, 85, 0).withTimeout(1.5), // Drive to 45 inches
        new WaitCommand(1),
        new InstantCommand(() -> {
            m_swerveDrive.resetDistanceMotors(); // Reset distance motors (CURCIAL TO DRIVING)
        }),
        new PIDGyroCommand(0, m_swerveDrive).withTimeout(.5),
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "intakeOn", Constants.MAX_SHOOTER_ANGLE),
        new PIDCompositionDriveCommand(m_swerveDrive, 4, 65, -10).until(m_DeviceSubsystem::checkRing).withTimeout(1.5), // Drive to 45 inches,
        new PIDGyroCommand(0, m_swerveDrive).withTimeout(.5),

        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "intakeOff", Constants.MAX_SHOOTER_ANGLE).withTimeout(0.25), // Turn intake off
        new PIDGyroCommand(-45, m_swerveDrive).withTimeout(.5),
...
public static Command Blue_SCB(DeviceSubsystem m_DeviceSubsystem, ShooterAnglePIDSubsystem m_AnglePIDSubsystem, LimelightSubsystem m_limelight, SwerveSubsystem m_swerveDrive) {
    return new SequentialCommandGroup(
        ...
        new PIDCompositionDriveCommand(m_swerveDrive, 7.5, 50, 0).withTimeout(1.5), // Drive to 45 inches
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "intakeOff", Constants.MAX_SHOOTER_ANGLE).withTimeout(0.25), // Turn intake off
        new PIDGyroCommand(-25, m_swerveDrive).withTimeout(.5), // Turn to 22 degrees
        new PIDCommandTurnToAngle(m_limelight, m_swerveDrive).withTimeout(0.2), // Align to april tag
        new WaitCommand(1), // Wait to allow time for arm to adjust
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "outtake", Constants.DEFAULT_SHOOTER_ANGLE), // Outtake a little
        new ShootSequence(m_DeviceSubsystem).withTimeout(1.7), // Shoot
        ...
        new WaitCommand(.2),
        new PIDCompositionDriveCommand(m_swerveDrive, -115, 70, 115).withTimeout(2), // Drive to 45 inches
        // new InstantCommand(() -> {
        //     m_swerveDrive.resetDistanceMotors(); // Reset distance motors (CURCIAL TO DRIVING)
        // }),
        // new PIDCompositionDriveCommand(m_swerveDrive, 0, 50, 0).withTimeout(1.5), // Drive to 45 inches
        new PIDGyroCommand(0, m_swerveDrive).withTimeout(1.25),
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "intakeOff", Constants.MAX_SHOOTER_ANGLE).withTimeout(0.25), // Turn intake off
        new WaitCommand(1), // Wait to allow time for arm to adjust
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "outtake", Constants.DEFAULT_SHOOTER_ANGLE), // Outtake a little
        new ShootSequence(m_DeviceSubsystem).withTimeout(1.7), // Shoot
...
public static Command Blue_SBA(DeviceSubsystem m_DeviceSubsystem, ShooterAnglePIDSubsystem m_AnglePIDSubsystem, LimelightSubsystem m_limelight, SwerveSubsystem m_swerveDrive) {
    ...
    new ShootSequence(m_DeviceSubsystem).withTimeout(2.4),
        ...
        new PIDCompositionDriveCommand(m_swerveDrive, 130, 70, 0).withTimeout(1.5), // Drive to 45 inches
        new InstantCommand(() -> {
            m_swerveDrive.resetDistanceMotors(); // Reset distance motors (CURCIAL TO DRIVING)
        }),
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "intakeOn", Constants.MAX_SHOOTER_ANGLE), // Turn on intake
            new PIDGyroCommand(0, m_swerveDrive).withTimeout(.25),
...
public static Command Red_SJI(DeviceSubsystem m_DeviceSubsystem, ShooterAnglePIDSubsystem m_AnglePIDSubsystem, LimelightSubsystem m_limelight, SwerveSubsystem m_swerveDrive) {
    return new SequentialCommandGroup(
        ...
        new WaitCommand(1), // Wait to allow time for arm to adjust
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "outtake", Constants.DEFAULT_SHOOTER_ANGLE), // Outtake a little
        new ShootSequence(m_DeviceSubsystem).withTimeout(1.7),
        new WaitCommand(10) // Shoot
    );
}
...
public static Command Red_SJK(DeviceSubsystem m_DeviceSubsystem, ShooterAnglePIDSubsystem m_AnglePIDSubsystem, LimelightSubsystem m_limelight, SwerveSubsystem m_swerveDrive) {
    return new SequentialCommandGroup(
        ...
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "intakeOn", Constants.MAX_SHOOTER_ANGLE),
        new PIDCompositionDriveCommand(m_swerveDrive, 0, 55, 0).withTimeout(1.5), // Drive to 45 inches
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "intakeOff", Constants.MAX_SHOOTER_ANGLE).withTimeout(0.25), // Turn intake off
        new PIDGyroCommand(22, m_swerveDrive).withTimeout(.5),
        new PIDCommandTurnToAngle(m_limelight, m_swerveDrive).withTimeout(.1),
        new WaitCommand(1), // Wait to allow time for arm to adjust
...
public static Command RedorBlue_SABC_or_SKJI(DeviceSubsystem m_DeviceSubsystem, ShooterAnglePIDSubsystem m_AnglePIDSubsystem, LimelightSubsystem m_limelight, SwerveSubsystem m_swerveDrive) {
    return new SequentialCommandGroup(
        ...
        //new PIDCommandTurnToAngle(m_limelight, m_swerveDrive).withTimeout(.25),
        new ParallelCommandGroup(
            new PIDCompositionDriveCommand(m_swerveDrive, 130, 87, 0).withTimeout(1.5), // Move over 66 inches
            new ShootSequence(m_DeviceSubsystem).withTimeout(2.4)
        ).withTimeout(2.5),
        new PIDGyroCommand(0, m_swerveDrive).withTimeout(.2),
        ...
        new PIDGyroCommand(45, m_swerveDrive).withTimeout(.5),
        new PIDCompositionDriveCommand(m_swerveDrive, 45, 75, 40).withTimeout(1.5), // Move over 66 inches
        new IntakeSequence(m_DeviceSubsystem, m_AnglePIDSubsystem, "intakeOff", Constants.MAX_SHOOTER_ANGLE),
        new InstantCommand(() -> {
            m_swerveDrive.resetDistanceMotors(); // Reset distance motors (CURCIAL TO DRIVING)
        }),
        ...
        new PIDGyroCommand(0, m_swerveDrive).withTimeout(.2),
        new ParallelCommandGroup(
            new PIDCompositionDriveCommand(m_swerveDrive, -130, 75, 0).withTimeout(1.5), // Move over 66 inches
            new ShootSequence(m_DeviceSubsystem).withTimeout(2.4)
        ),
        new InstantCommand(() -> {
            m_swerveDrive.resetDistanceMotors(); // Reset distance motors (CURCIAL TO DRIVING)
        }),
        new PIDCompositionDriveCommand(m_swerveDrive, 0, 40, 0),
